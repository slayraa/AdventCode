...
for i in range(len(galax_coordinates)-1):
    for j in range(i+1, len(galax_coordinates)):
        ...
        if [row1, col1] != [row2, col2]:
            ...
            distance = np.linalg.norm(galax_end - galax_start, 1) + len(rows_crossed)*(expansion-1) + len(cols_crossed)*(expansion-1)
...
# Sample for 10x is 1030
# Sample for 100x is 8410
# 504715068438