...
connected_boxes = []
missing_boxes = jct_boxes.copy()

pair_i = 0
while len(missing_boxes) > 0 and pair_i < len(n_closest_pairs):
    pair = n_closest_pairs[pair_i]
    pair_i += 1
    missing_boxes = np.array([box for box in missing_boxes if not (np.array_equal(box, pair[0]) or np.array_equal(box, pair[1]))])
    ...
    if len(connected_boxes) == 1 and len(missing_boxes) == 0:
        print(f"Stopped after {pair_i-1} pairs, with pair {pair}")
        pair_i = len(n_closest_pairs)  # Exit loop

x_coords = [pair[0][0], pair[1][0]]

print(f"What do you get if you multiply together the X coordinates of the last two junction boxes you need to connect? {np.prod(x_coords)}")
# sample = 25272
# 6083499488